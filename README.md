...
- **浏览器操作**（`src/tools/browser/navigate.py`）：基于 Playwright——导航、点击（CSS/文本/角色策略）、输入、截图、DOM 摘要、文本提取、图片上传和页面状态复验
- **AI 工具**（`src/tools/ai/`）：文章/配图生成、文本摘要、Agnes 屏幕分析和结构化 UI 目标定位
...
| **知乎网页闭环** | 已接入文章生成、配图落盘/上传、持久化浏览器上下文、发布与互动状态审计 | 已完成模拟页面单元测试，尚未使用真实账号执行发布、搜索及互动测试 |
...
- **知乎**：登录、写文、生成/上传配图、发布、搜索、评论和互动状态复验（真实账号端到端测试仍缺失）
